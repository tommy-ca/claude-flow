...
app.use((err, req, res, next) => {
  ...
  logger.error('REST API server error', { error: err.message, stack: err.stack });
...
app.listen(port, () => {
  const logger = new Logger();
  logger.info('REST API server started', { port, creator: 'Claude Flow Swarm' });
});
...
// Application entry point - generated by Claude Flow Swarm
// Swarm ID: ${this.swarmId.id}
// Task: ${task.name} 
