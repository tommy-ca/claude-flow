...
import { IResourceMetrics, IResourceThresholds, IResourceAlert, ResourceType } from './metrics';
...
export interface IResourceMonitor {
  ...
  getHistoricalMetrics(startTime?: Date, endTime?: Date): Promise<IResourceMetrics[]>;

  /** Get average metrics over a time period */
  getAverageMetrics(duration: number): Promise<IResourceMetrics>;
  ...
  updateConfig(config: Partial<IResourceMonitorConfig>): void;
  ...
  /** Get alerts history */
  getAlertHistory(limit?: number): IResourceAlert[];

  /** Clear alert history */
  clearAlertHistory(): void;

  /** Export metrics to file or stream */
  exportMetrics(format: ExportFormat, destination: string | NodeJS.WritableStream): Promise<void>;
}
...
export interface IResourceMonitorFactory {
  ...
  create(config: IResourceMonitorConfig): IResourceMonitor;

  /** Register a monitoring strategy */
  registerStrategy(strategy: IMonitoringStrategy): void;

  /** Register an alert handler */
  registerAlertHandler(handler: IAlertHandler): void;

  /** Register a metric exporter */
  registerExporter(exporter: IMetricExporter): void;
}
